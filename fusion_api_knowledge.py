...
API_CARDS = [
    ...
    },
    {
        ...
        "content": (
            ...
            "CRITICAL — EDGE-FIRST APPROACH:\n"
            "- ALWAYS iterate body.edges DIRECTLY to find target edges.\n"
            "- NEVER search for a face first and then get its edges. This fails on shelled/curved bodies.\n"
            "- NEVER raise SELECTION_REQUIRED for a face when doing fillets — fillets need EDGES.\n"
            "- Use edge.pointOnEdge (midpoint) and edge.geometry type (Circle3D, Arc3D, Line3D) to classify.\n\n"
            ...
            "- NEVER search for faces by normal and then get face.edges — iterate body.edges directly.\n\n"
    ...
    },
    {
        ...
        "content": (
            "[shell-feature] Hollow a body by removing a face and setting wall thickness:\n"
            "\n"
            "OPTION A — User has selected a face (ui.activeSelections.count > 0):\n"
            "1. `faceToRemove = adsk.fusion.BRepFace.cast(ui.activeSelections.item(0).entity)`\n"
            "\n"
            "OPTION B — Find face programmatically (when description specifies which face):\n"
            "  Top face:    find face whose centroid has max Z AND normal.z > 0.9\n"
            "  Bottom face: find face whose centroid has min Z AND normal.z < -0.9\n"
            "  Pattern:\n"
            "```\n"
            ...
            "    f = body.faces.item(i)\n"
            "    if hasattr(f.geometry, 'normal'):  # planar faces only\n"
            "        n = f.geometry.normal\n"
            "        centroid = f.centroid\n"
            "        if n.z > 0.9 and centroid.z > maxZ:  # top face\n"
            "            maxZ = centroid.z\n"
            "            faceToRemove = f\n"
            "if not faceToRemove:\n"
            "    raise Exception('Could not find top face to shell')\n"
            "```\n"
            "\n"
            ...
            "The specified face is the one that gets REMOVED (opened up). All other faces become thin walls.\n"
            "NEVER create the inputFaces collection without adding a face — an empty collection shells with no opening."
        ),
        "sources": [
    ...
    },
    {
        ...
        "content": (
            ...
            "- `inputFeatures = adsk.core.ObjectCollection.create(); inputFeatures.add(seedFeature)`\n"
            "AXIS SELECTION — always use a construction axis, NEVER derive from a face:\n"
            "- Holes/slots on bottom face of Z-extruded body → `axis = rootComp.zConstructionAxis`\n"
            "- Holes on top/bottom face → `axis = rootComp.zConstructionAxis`\n"
            "- Holes on front/back face → `axis = rootComp.yConstructionAxis`\n"
            "- Holes on left/right face → `axis = rootComp.xConstructionAxis`\n"
            "- NEVER try to find a face and extract its normal as an axis — use construction axes.\n"
            "- `patInput = patFeats.createInput(inputFeatures, axis)`\n"
            ...
            "- `patFeats.add(patInput)`\n"
            "FINDING SEED FEATURE — use feature history, not face search:\n"
            "- Last extrude cut: `seedFeat = rootComp.features.extrudeFeatures.item(rootComp.features.extrudeFeatures.count - 1)`\n"
            "- Last hole: `seedFeat = rootComp.features.holeFeatures.item(rootComp.features.holeFeatures.count - 1)`"
        ),
        "sources": [
            "https://help.autodesk.com/cloudhelp/ENU/Fusion-360-API/files/CircularPatternFeatures_createInput.htm",
        ],
